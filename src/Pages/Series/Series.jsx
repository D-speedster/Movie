...
export default function Series() {
    ...
    let [filteredSeries, setFilteredSeries] = useState([]);
    ...
    const SearchBtn = () => {
        ...
        if (filters.rating) {
            ...
            filtered = filtered.filter(series => parseFloat(series.rate) >= minRating);
        }
    ...
    useEffect(() => {
        ApiRequest.get('/Series').then(Response => {
            setAll_Moviez(Response.data);
            setFilteredSeries(Object.entries(Response.data).map(item => item[1]));
        }).catch(error => {
